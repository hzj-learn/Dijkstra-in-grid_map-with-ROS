...
Point startPoint, targetPoint;  //定义起始点和目标点
...
void MapCallback(const nav_msgs::OccupancyGrid& msg)
{
    ...
    for(int i=0;i<height;i++)
    {
        for(int j=0;j<width;j++)
        {
            ...
            //OccGrid的起源在地图的左下角 The origin of the OccGrid is on the bottom left corner of the map
...
{
    //  Initial node 
    ros::init(argc, argv, "diagonal");//创建一个节点
    ros::NodeHandle nh;
    ...
    startpoint_flag = false;
    targetpoint_flag = false;
    start_flag = false;//控制是否启动dijkstra规划,同时收到起始点和目标点后才开始规划，规划一次后不再规划

    // Parameter
    nh_priv.param<bool>("Diagonal", config.Diagonal, true);
    nh_priv.param<int>("OccupyThresh", config.OccupyThresh, -1);
    nh_priv.param<double>("InflateRadius", InflateRadius, -1);//膨胀半径
    nh_priv.param<int>("rate", rate, 10);

    // Subscribe topics
    map_sub = nh.subscribe("map", 10, MapCallback);//占据栅格地图，map_sever这个功能包发布的
    startPoint_sub = nh.subscribe("initialpose", 10, StartPointCallback);//起始位置，rviz发布的
    targetPoint_sub = nh.subscribe("move_base_simple/goal", 10, TargetPointtCallback);//终止位置，rviz发布的

    // Advertise topics
    mask_pub = nh.advertise<nav_msgs::OccupancyGrid>("mask", 1);//边缘
    path_pub = nh.advertise<nav_msgs::Path>("nav_path", 10);//路径
    ...
    while(ros::ok())
    {
        ...
        {
            // Start planning path
            vector<Point> PathList;//路径点集合
            dijkstra.PathPlanning(startPoint, targetPoint, PathList);//dijkstra算法的实现
            if(!PathList.empty())//搜索到路径
            {
                ...
                {
                    Point2d dst_point;
                    OccGridParam.Image2MapTransform(PathList[i], dst_point);//将每一个点转换成rviz发布的Point2d数据类型
